...
```{r}
#create bins for which waterdrop image will coorespond to which percentage 
# can change this depending on 

get_waterdrop_label <- function(percent) {
  bins <- c(0.9, 4, 7, 9, 10,
            12.9, 14.9, 17.9, 19.9, 22.9, 24.9, 27.9, 29.9, 32.9,
            34.9, 37.9, 39.9, 42.9, 44.9, 47.9, 49.9, 52.9, 54.9,
            57.9, 59.9, 62.9, 64.9, 67.9, 69.9, 72.9, 74.9, 77.9,
            79.9, 82.9, 84.9, 87.9, 89.9, 92.9, 94.9, 97.9, 99.9, 100.1)

  labels <- c("00", "02", "05", "08", "10",
              "12", "15", "18", "20", "23", "25", "28", "30", "33",
              "35", "38", "40", "43", "45", "48", "50", "53", "55",
              "58", "60", "63", "65", "68", "70", "73", "75", "78",
              "80", "83", "85", "88", "90", "93", "95", "98", "100", "100")
...
# Read Northern Sierra PDF text
northern_pdf_url <- "https://cdec.water.ca.gov/cgi-progs/products/PLOT_ESI.pdf"
northern_pdf_text <- pdf_text(northern_pdf_url)
combined_text <- paste(northern_pdf_text, collapse = " ")

# Extract precipitation (inches)
precip_pattern <- "(?i)Current:\\s?(\\d{1,4}(\\.\\d{1,2})?)"
precip_match <- str_match(combined_text, precip_pattern)
precip_value <- as.numeric(precip_match[2])

# Extract percent of average
percent_pattern <- "(?i)Percent of Average for this Date:\\s*(\\d+%)"
percent_match_text <- regmatches(combined_text, regexpr(percent_pattern, combined_text, perl = TRUE))

if (!is.na(precip_value) && length(percent_match_text) > 0) {

  percent_value <- sub(percent_pattern, "\\1", percent_match_text)
  numeric_percent <- as.numeric(str_remove(percent_value, "%"))

  # Color coding
  color <- ifelse(numeric_percent >= 90, "green",
           ifelse(numeric_percent >= 75, "blue",
           ifelse(numeric_percent >= 50, "orange", "red")))

  # Formatted text
  formatted_output <- sprintf(
    "<strong><span style='color:%s'>%.1f inches</span></strong><br>
     Percent of average rain for this date<br>
     <strong><span style='color:%s'>%s</span></strong><br>",
    color, precip_value, color, percent_value
  )

  # Determine image
  label <- get_waterdrop_label(numeric_percent)
  file_path <- sprintf("images/waterdrops/waterdrop_%s.png", label)

  image_html <- sprintf(
    "<div style='margin: 4px 0; line-height: 1;'>
       <img src='%s' width='220px' style='display: block; margin: 0; padding: 0;' alt='Waterdrop for %s%% precipitation'>
     </div>", file_path, numeric_percent
  )

  HTML(paste0(formatted_output, image_html))

} else {
  cat("Northern Sierra precipitation data not available.\n")
}
```
...
# Read Central Sierra PDF text
central_pdf_url <- "https://cdec.water.ca.gov/cgi-progs/products/PLOT_FSI.pdf"
central_pdf_text <- pdf_text(central_pdf_url)
combined_text <- paste(central_pdf_text, collapse = " ")
...
if (!is.na(precip_value) && length(percent_match_text) > 0) {
  ...
  cat("Central Sierra precipitation data not available.\n")
}
```
...
```{r}
# Extract and render snowflake image based on percent
if (length(north_percentages) >= 2) {
  percent_value <- as.numeric(str_remove(north_percentages[2], "%"))

  # Round down to nearest 5%
  percent_rounded <- floor(percent_value / 5) * 5
  percent_rounded <- min(percent_rounded, 95)  # cap at 95 to stay in range

  # Build file path
  file_path <- sprintf("images/snowflakes/snowflake_%02d.png", percent_rounded)

  # Render the image with caption
  flake_img_tag <- sprintf(
    "<img src='%s' width='150px' alt='Snowflake for %d%% snowpack'>",
    file_path, percent_rounded
  )

  output_html <- paste0(
    "Northern Sierra snowpack level: ", north_percentages[2], "<br>",
    flake_img_tag
  )
...
# Extract and render snowflake image based on percent
if (length(central_percentages) >= 2) {
  percent_value <- as.numeric(str_remove(central_percentages[2], "%"))
  ...
  output_html <- paste0(
    "Central Sierra snowpack level: ", central_percentages[2], "<br>",
    flake_img_tag
  )
  ...
  cat("Second percentage in the 'CENTRAL' section not found.")
}
...
# Extract and render snowflake image based on percent
if (length(statewide_percentages) >= 2) {
  percent_value <- as.numeric(str_remove(statewide_percentages[2], "%"))
  ...
  output_html <- paste0(
    "State wide snowpack level: ", statewide_percentages[2], "<br>",
    flake_img_tag
  )
  ...
  cat("Second percentage in the 'STATE' section not found.")
}
...
leaflet() %>%
  addTiles() %>%
  setView(lng = -121.5, lat = 39.0, zoom = 6.25) %>%  # Center the map
  ...
  addMarkers(lng = 	-122.420000, lat = 40.718000, popup = "Shasta Reservoir") %>%    # Shasta reservoir (SHA)
...
##### Shaded area on map is the legal delta. Points are reservoir locations
...
leaflet() %>%
  ...
  addMarkers(lng = 	-121.265002, lat = 37.679376, popup = "San Joaquin River near Vernalis") %>% # Add specific gps points
  addMarkers(lng = -121.500300, lat = 	38.456112, popup = "Sacramento at Freeport") %>%          # Add specific gps points
...
# Your reservoir data (dynamically pulled earlier)
reservoirs <- tibble(
  Reservoir = c("Shasta", "Oroville", "San Luis"),
  `Percent of Capacity` = c(
    shasta_percent_capacity_value,
    oroville_percent_capacity_value,
    SanLuis_percent_capacity_value
  ),
  `Percent of Average` = c(
    shasta_percent_average_storage_value,
    oroville_percent_average_storage_value,
    SanLuis_percent_average_storage_value
  )
)

# Reshape to long format
reservoirs_long <- pivot_longer(reservoirs, -Reservoir, names_to = "Metric", values_to = "Percent")

# Assign text colors based on "Percent of Average" values only
reservoirs_long <- reservoirs_long %>%
  mutate(text_color = case_when(
    Metric == "Percent of Average" & Percent >= 90 ~ "darkgreen",
    Metric == "Percent of Average" & Percent >= 75 ~ "blue",
    Metric == "Percent of Average" & Percent >= 50 ~ "orange",
    Metric == "Percent of Average" ~ "red",
    TRUE ~ "black"  # Keep other metrics (like Capacity) with default text color
  ))

# Plot
ggplot(reservoirs_long, aes(x = Reservoir, y = Percent, fill = Metric)) +
  geom_col(position = position_dodge(width = 0.6), width = 0.6) +
  geom_text(aes(label = paste0(Percent, "%"), color = text_color, group = Metric),
            position = position_dodge(width = 0.6),
            vjust = -0.5,
            size = 4,
            fontface = "bold",  # <-- this makes it bold
            show.legend = FALSE) +
  scale_color_identity() +
  labs(
    title = "Reservoir Percent Average Storage and Capacity",
    y = "Percent of average for this Date",
    x = ""
  ) +
...
<div style='margin-top: 16px; font-weight: bold;'>
  Percent of 20-year flow average:
  <br>
